...
const Reader: React.FC = () => {
  ...
  return (
    // Fixed positioning to fill the screen
    // Mobile: top-0 because we hid the top navbar via Navbar.tsx logic? No, Navbar still shows top on Desktop. 
    // Actually, we usually want full screen on mobile.
    // Let's use flex-col h-screen to make it behave like an app view.
    <div className="fixed inset-0 flex flex-col bg-white z-0">
      
      {/* Top Header Strip - Always Visible */}
      <div className="flex-none h-14 border-b border-slate-100 flex items-center justify-between px-4 bg-white z-20 shadow-sm safe-area-top">
          <button 
            onClick={() => navigate('/')} 
            className="flex items-center text-slate-500 hover:text-teal-700 transition-colors py-2"
          >
          ...
          <div className="flex flex-col items-center">
             <span className="text-sm font-bold text-slate-800 font-arabic">{getPageTitle()}</span>
             {getJuzNumber() && <span className="text-[10px] text-slate-400 uppercase tracking-widest">Juz {getJuzNumber()}</span>}
          </div>
      ...
      <div className="flex-grow overflow-y-auto bg-white relative flex justify-center py-4 md:py-6 scroll-smooth">
        {loading ? (
            ...
            <div className="w-full max-w-[500px] md:max-w-[650px] px-4 md:px-0 mx-auto pb-20">
                 <div className="bg-white min-h-[80vh] relative">
                                    ...
                                    <React.Fragment key={verse.id}>
                                        {showBismillah && (
                                            <div className="w-full text-center py-6 mb-2 select-none">
                                                <span className="font-arabic text-3xl md:text-4xl text-teal-900 opacity-90 block" style={{ fontFamily: 'Amiri' }}>
                     ...
                     <div className="text-center py-6">
                        <span className="text-xs font-serif text-slate-400 font-bold">{currentPage}</span>
                     </div>
                 </div>
            </div>
        )}
      ...
      {/* Increased height and touch targets for mobile */}
      <div className="flex-none h-16 md:h-20 border-t border-slate-100 bg-white flex items-center justify-between px-6 z-30 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.02)] safe-area-bottom">
           <button 
             onClick={handlePrevPage}
             disabled={currentPage <= 1}
             className="flex items-center space-x-2 px-6 py-3 rounded-full hover:bg-slate-50 disabled:opacity-30 disabled:hover:bg-transparent text-teal-800 transition-colors active:scale-95"
           >
           ...
           <button 
             ...
             className="flex items-center space-x-2 px-6 py-3 rounded-full bg-teal-50 hover:bg-teal-100 disabled:opacity-30 disabled:bg-transparent text-teal-800 transition-colors active:scale-95"
           >
      ...
      {selectedVerse && (
        <div className="absolute bottom-24 left-1/2 transform -translate-x-1/2 w-auto max-w-[95%] z-40">
            <div className="bg-slate-900/95 backdrop-blur-md text-white px-5 py-4 rounded-2xl shadow-2xl flex items-center gap-4 animate-in slide-in-from-bottom-5 duration-300 border border-white/10">
               ...
               >
                 <Heart className={`w-6 h-6 ${isFavorite(selectedVerse.verse_key) ? 'fill-current' : ''}`} />
               </button>
